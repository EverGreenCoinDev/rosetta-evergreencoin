...
GO_PACKAGES=./services/... ./indexer/... ./evergreencoin/... ./configuration/...
...
build:
	docker build -t rosetta-evergreencoin:latest https://github.com/evergreencoindev/rosetta-evergreencoin.git

build-local:
	docker build -t rosetta-evergreencoin:latest .

build-release:
	# make sure to always set version with vX.X.X
	docker build -t rosetta-evergreencoin:$(version) .;
	docker save rosetta-evergreencoin:$(version) | gzip > rosetta-evergreencoin-$(version).tar.gz;

run-mainnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/evergreencoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 5758:5758 rosetta-evergreencoin:latest

run-mainnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-evergreencoin:latest

run-testnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/evergreencoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 15758:15758 rosetta-evergreencoin:latest
